  ...
  root.innerHTML = `
    <main style="min-height:100vh;display:grid;place-items:center;padding:24px;background:#f4f1e7;font-family:system-ui,sans-serif">
      <section role="alert" style="width:min(100%,420px);padding:28px;border:1px solid #ded8c7;border-radius:20px;background:#fff;text-align:center;color:#17231f">
        <h1 style="margin:0 0 8px;font-size:22px">Não foi possível abrir o EduStock Cozinha</h1>
        <p style="margin:0 0 20px;color:#63706a">Um arquivo antigo do aplicativo pode estar armazenado no navegador.</p>
        <button id="recarregar-app" type="button" style="width:100%;padding:14px;border:0;border-radius:14px;background:#e07a3e;color:#fff;font-size:16px;font-weight:700;cursor:pointer">Recarregar aplicativo</button>
      </section>
    </main>
  `
...
async function limparResiduosDeDesenvolvimento() {
  ...
  if ('caches' in globalThis) {
    ...
    await Promise.all(
      chaves
        .filter((chave) => chave.startsWith('edustock-cozinha-'))
...
async function iniciar() {
  try {
    ...
    console.error('Falha ao iniciar o EduStock Cozinha.', error)
