    ...
    def aplicar(self, request, pk=None):
        ...
        for item in modelo.ingredientes:
            ...
            produto, _ = Produto.objects.get_or_create(
                ...
                defaults={
                    ...
                    **_conversao_padrao(catalogo.unidade),
            ...
            if not produto.unidade_consumo and produto.conteudo_por_unidade is None:
                conversao = _conversao_padrao(produto.unidade)
                if conversao:
                    produto.unidade_consumo = conversao["unidade_consumo"]
                    produto.conteudo_por_unidade = conversao["conteudo_por_unidade"]
            ...
            if not produto.unidade_consumo or produto.conteudo_por_unidade is None:
                raise serializers.ValidationError({
                    "detail": (
                        f"Configure a conversão de unidade do produto '{produto.nome}' "
                        "antes de aplicar o modelo."
                    )
                })
            quantidade = item.get("quantidade_por_aluno", item.get("gramas_por_aluno"))
            ingredientes.append((produto, _decimal(quantidade)))
        ...
        ReceitaIngrediente.objects.bulk_create([
            ReceitaIngrediente(
                receita=receita,
                produto=produto,
                quantidade_por_aluno=quantidade,
            )
...
def _conversao_padrao(unidade):
    conversoes = {
        "KG": {"unidade_consumo": "G", "conteudo_por_unidade": Decimal("1000")},
        "L": {"unidade_consumo": "ML", "conteudo_por_unidade": Decimal("1000")},
        "UN": {"unidade_consumo": "UN", "conteudo_por_unidade": Decimal("1")},
    }
    return conversoes.get(unidade, {})
