...
export default function PwaControls({ pwa }) {
  return (
    <section className="mt-7 w-full border-t border-line pt-5" aria-label="Aplicativo e conexão">
      ...
      {pwa.canInstall && (
        <button type="button" onClick={pwa.install} className="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl border border-brand bg-brand-tint px-4 py-3 font-bold text-brand">
          <DownloadSimple size={21} weight="bold" /> Instalar EduStock Alunos
        </button>
      )}

      {pwa.updateAvailable && (
        <button type="button" onClick={pwa.applyUpdate} className="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl bg-brand px-4 py-3 font-bold text-white">
      ...
      {!pwa.canInstall && !pwa.installed && (
        <details className="mt-3 rounded-2xl bg-surface-2 px-4 py-3 text-sm text-ink-soft">
          <summary className="flex cursor-pointer list-none items-center justify-center gap-2 font-bold text-brand">
